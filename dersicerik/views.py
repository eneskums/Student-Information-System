...
def dersicerikTalep(request):
    ...
    if formMezun.is_valid():
        ...
        return render(request, 'dersicerik/dersicerik.html',
                      {'tarih': tarih, 'formMezun': formMezun, 'ogretimGorevlisi': ogretimGorevlisi})
...
def dersicerikIstekleri(request):
    for g in request.user.groups.all():
        ...
        if g.name == 'Öğretim Görevlisi':
            ...
            hoca.save()
            return render(request, 'dersicerik/dersicerikIstekleri.html',
                          {'mezunIstekleri': mezunIstekleri, 'hoca': hoca})
