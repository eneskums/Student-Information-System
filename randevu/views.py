...
def randevu_create(request):
    ...
    if request.method == "POST":
        ...
        for grup in request.user.groups.all():
            ...
            if grup.name == 'Öğrenci':
                form = RandevuFormOgrenci(request.POST)
                return render(request, 'randevu/create.html',
                              {'tarih': tarih, 'form': form, 'ogretimGorevlisi': ogretimGorevlisi})
            if grup.name == 'Mezun':
                form = RandevuFormMezun(request.POST)
                return render(request, 'randevu/create.html',
                              {'form': form, 'tarih': tarih, 'ogretimGorevlisi': ogretimGorevlisi})
...
def randevu_list(request):
    for grup in request.user.groups.all():
        ...
        if grup.name == 'Öğretim Görevlisi':
            ...
            hoca.save()
            return render(request, 'randevu/list.html',
                          {'ogrenciRandevu': ogrenciRandevu, 'mezunRandevu': mezunRandevu})
    ...
    for g in request.user.groups.all():
        ...
        if g.name == 'Öğrenci' and istek == "0":
            ...
            return render(request, 'randevu/detay.html',
                          {'randevuOgrenci': ogrenciRandevu, 'form': form, 'tarih': tarih})
        ...
        if g.name == 'Öğretim Görevlisi':
            if istek == "0":
                ...
                if form.is_valid():
                    ...
                    return redirect('randevu:detay', ogrenciRandevu.pk, 0)
                return render(request, 'randevu/detay.html',
                              {'randevuOgrenci': ogrenciRandevu, 'form': form, 'tarih': tarih, 'istek': istek})
                ...
                if form.is_valid():
                    ...
                    return redirect('randevu:detay', mezunRandevu.pk, 1)
                return render(request, 'randevu/detay.html',
                              {'randevuMezun': mezunRandevu, 'form': form, 'tarih': tarih, 'istek': istek})
            return render(request, 'randevu/detay.html')
